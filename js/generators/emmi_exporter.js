    ...
    class EMMIScriptExporter {
        ...
        serializeStatementBlock(block) {
            ...
            switch (block.type) {
                ...
                // ── New buzzer_* blocks (mapped to same B-tokens) ──────────
                case 'buzzer_play_rtttl': {
                    this.addInitToken('B');
                    const mel = block.getFieldValue('MELODY') || 'StarWars';
                    return ['BP' + mel];
                }
                case 'buzzer_play_rtttl_custom': {
                    this.addInitToken('B');
                    const customMel = this.serializeValueInput(block, 'MELODY', 'S') || '"StarWars"';
                    return ['BP' + customMel];
                }
                case 'buzzer_play_note': {
                    this.addInitToken('B');
                    // NOTE field stores frequency (e.g. "440"), TEMPO stores duration (ms)
                    const noteFreq = block.getFieldValue('NOTE') || '440';
                    const noteTempo = block.getFieldValue('TEMPO') || '250';
                    return ['BF' + noteFreq, this.mapDelayValue(noteTempo), 'BS'];
                }
                case 'buzzer_play_tone': {
                    this.addInitToken('B');
                    const toneFreq = this.serializeValueInput(block, 'FREQ', 'F') || '440';
                    const toneDur = this.serializeValueInput(block, 'DURATION', 'I') || '100';
                    return ['BF' + toneFreq, this.mapDelayValue(toneDur), 'BS'];
                }
                case 'buzzer_stop':
                    this.addInitToken('B');
                    return ['BS'];
                ...
                case 'custom_wait': {
                    ...
                    let value = block.getFieldValue('DELAY') || '0';
                ...
                case 'ifandifnot': {
                    ...
                    // Build else block (if any)
                    if (block.getInputTargetBlock('ELSE')) {
                        elseTokens = this.serializeChain(this.getInputTarget(block, 'ELSE'));
                    } else if (block.elseifCount_) {
                        // Future handling for multiple else-if (requires nested C)
                        // For now, EMMI script only supports single if/else easily.
                    }
                ...
                case 'controls_repeat_ext': {
                    const times = this.serializeValueInput(block, 'TIMES', 'I') || '1';
                    const body = this.serializeChain(this.getInputTarget(block, 'DO'));
                    // EMMI scripting doesn't have a direct "repeat N times" without a variable, 
                    // so we compile it down to a for loop using a temporary dummy var or just use F(1-times,1)
                    return ['F(1-' + times + ',1){' + this.wrapBody(body) + '}'];
                }
                case 'controls_for': {
                ...
                case 'controls_forEach': {
                    // EMMI script doesn't natively support arrays/foreach easily, but we'll try to map it gracefully
                    // or return an empty block if unsupported. For now, wrap body in a loop.
                    const list = this.serializeValueInput(block, 'LIST', 'A') || '';
                    const body = this.serializeChain(this.getInputTarget(block, 'DO'));
                    return ['F(1-5,1){' + this.wrapBody(body) + '}']; // fallback for EMMI
                }
                case 'controls_switch': {
                    const value = this.serializeValueInput(block, 'SWVAR') || '0'; // Note SWVAR instead of SWITCH_VALUE
                    const caseValue = this.serializeValueInput(block, 'CASE0') || '0';
                    const body = this.serializeChain(this.getInputTarget(block, 'DO0'));

                    let extraCases = '';
                    for (let i = 1; i <= block.casebreakCount_; i++) {
                        const cv = this.serializeValueInput(block, 'CASE' + i) || '0';
                        const cb = this.serializeChain(this.getInputTarget(block, 'DO' + i));
                        extraCases += '(' + cv + '){' + this.wrapBody(cb) + '} ';
                    }

                    let defBody = '';
                    if (block.defaultCount_) {
                        defBody = this.wrapBody(this.serializeChain(this.getInputTarget(block, 'DEFAULT')));
                    }

                    return ['K(' + value + ', (' + caseValue + '){' + this.wrapBody(body) + '} ' + extraCases + '(D){' + defBody + '})'];
                }
        ...
        serializeValueInput(block, inputName, expectedType) {
            ...
            if (!target) return '';
            const val = this.serializeValueBlock(target, expectedType || null);

            // If the block is expected to be a condition (like IF0) and isn't an expression
            // we should wrap it in an equality check, because EMMI script requires conditionals
            // to evaluate via O operator, e.g., C(O==,TR,1) instead of C(TR)
            if (expectedType === 'Boolean' || inputName.startsWith('IF') || inputName === 'BOOL') {
                if (!val.startsWith('O') && val !== '0' && val !== '1' && val !== 'true' && val !== 'false') {
                    return 'O==,' + val + ',1';
                }
            }
            return val;
        ...
        serializeValueBlock(block, expectedType) {
            ...
            switch (block.type) {
                ...
                case 'math_modulo': {
                    const a = this.serializeValueInput(block, 'DIVIDEND') || '0';
                    const b = this.serializeValueInput(block, 'DIVISOR') || '1';
                    return 'O%,' + a + ',' + b;
                }
                case 'math_single':
                case 'math_trig':
                case 'math_round': {
                    // EMMI doesn't have trig natively, represent as EMMI expression using variable value
                    const arg = this.serializeValueInput(block, 'NUM') || '0';
                    const op = block.getFieldValue('OP');
                    if (op === 'NEG') return 'O*,' + arg + ',-1';
                    if (op === 'ABS') return 'O*,1,' + arg; // best approximation for EMMI
                    return arg; // For trig/round, return raw value as EMMI can't evaluate these
                }
                case 'math_random_int': {
                    // EMMI doesn't have a native random, return the FROM value
                    return this.serializeValueInput(block, 'FROM') || '0';
                }
                case 'math_constant': {
                    const CONSTANTS = {
                        PI: '3.14159', E: '2.71828', GOLDEN_RATIO: '1.61803',
                        SQRT2: '1.41421', SQRT1_2: '0.70710', INFINITY: '32767'
                    };
                    return CONSTANTS[block.getFieldValue('CONSTANT')] || '0';
                }
                case 'math_number_property': {
                    const num = this.serializeValueInput(block, 'NUMBER_TO_CHECK') || '0';
                    const prop = block.getFieldValue('PROPERTY');
                    if (prop === 'EVEN') return 'O=,O%,' + num + ',2,0';
                    if (prop === 'ODD') return 'O!=,O%,' + num + ',2,0';
                    if (prop === 'POSITIVE') return 'O>,' + num + ',0';
                    if (prop === 'NEGATIVE') return 'O<,' + num + ',0';
                    return '0';
                }
                case 'intervalle': {
                    const inf = this.serializeValueInput(block, 'inf') || '0';
                    const val = this.serializeValueInput(block, 'valeur') || '0';
                    const sup = this.serializeValueInput(block, 'sup') || '0';
                    const compInf = block.getFieldValue('comp_inf');
                    const compSup = block.getFieldValue('comp_sup');
                    const opMap = { LT: '<', LTE: '<=', GT: '>', GTE: '>=' };
                    const leftOp = { LT: '>', GT: '<', GTE: '<=', LTE: '>=' }[compInf] || '>';
                    const rightOp = opMap[compSup] || '<';
                    return 'O*,O' + leftOp + ',' + val + ',' + inf + ',O' + rightOp + ',' + val + ',' + sup;
                }
                case 'text_join': {
                    // EMMI doesn't natively join, just serialize first item
                    if (!block.itemCount_) return '""';
                    return this.serializeValueInput(block, 'ADD0') || '""';
                }
                case 'text_length': {
                    return this.serializeValueInput(block, 'VALUE') || '""';
                }
        ...
        serializeArithmetic(block) {
            ...
            const op = opMap[block.getFieldValue('OP')];
            if (!op) {
                // POWER is not supported natively in EMMI; return left operand as fallback
